...
public class UpgradesShopHero : MonoBehaviour
{
    ...
    {
        _ = _sb.Clear();
        _ = _sb.Append(currentLevel);
        _ = _sb.Append('/');

        _ = _sb.Append(maxLevel);
        return _sb.ToString();
    }
    ...
    void UpdatePriceTextColor(TextMeshProUGUI text, string upgradeName)
    {
        ...
        text.colorGradientPreset = saveManager.money >= saveManager.GetHeroUpgrade(upgradeName).shopPrice ? priceOriginalColor : tooExpensivePriceColor;
    }
    ...
    {
        return selectedCard == "" ? null : transform.Find(Constants.HERO_BUTTON_PANEL_PATH + '/' + selectedCard);
    }
    ...
    void CreateUpgradeEffect()
    {
        ...
        effect.transform.position = selectCursor.transform.Find("CenterPosition").position;
    }
    ...
    public void UnlockUpgrade()
    {
        ...
        {
            _ = saveManager.unlockedHeroUpgrades.Remove(currentHeroUpgradeName);
            ...
            SetPlayerGoldText();
            SFXManager.instance.Play(upgradeSound, true, 0.05f);
