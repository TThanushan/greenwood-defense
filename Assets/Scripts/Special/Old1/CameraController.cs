...
public class CameraController : MonoBehaviour
{
    ...
    public void ReverseCameraAutoFollow()
    {
    ...
    //}
    bool IsEnemyTooCloseToTarget()
    {
        if (!unit)
            return false;

        GameObject newTarget = unit.GetClosestEnemy();
        float threshold = enemyDistanceThreshold;
        return !newTarget ? false : Vector2.Distance(target.transform.position, newTarget.transform.position) < threshold;
    }
    void UpdateCameraPosition()
    {
        if (!cameraAutoFollow || !target)
            return;
        smoothTime = IsEnemyTooCloseToTarget() ? slowSmoothTime : initialSmoothTime;
    ...
    public void ShakeWithDelay(float delay, float _intensity = 1, float _duration = 1)
    {
        ...
        _ = StartCoroutine(ShakeWithDelayIE(delay, _intensity, _duration));
    }
    ...
    void UpdateTarget()
    {
        ...
        {
            target = playerCaptainUnit.transform;
    ...
    GameObject GetFurthestAlly()
    {
        ...
        {
            if (ally.name.Equals("PlayerCaptain") || ally.GetComponent<Unit>().Disabled)
                continue;
